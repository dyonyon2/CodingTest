...
def check():
    global minAns
    global maxAns
    minTmp = [int(''.join(map(str,result))),minAns]
    maxTmp = [int(''.join(map(str,result))),maxAns]
    minAns = min(minTmp)
    maxAns = max(maxTmp)
...
def dfs():
    ...
    for i in range(num+1):
        if len(result) != 0:
            if i in result:
                continue
            if array[len(result)-1] == "<":
                if not (result[len(result)-1] < i):
                    continue
            else:
                if not (result[len(result)-1] > i):
                    continue
        ...
        result.pop()
dfs()
print(str(maxAns).zfill(count+1))
print(str(minAns).zfill(count+1))